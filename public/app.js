...
const fetchSystem = async () => {
  const system = await request("/api/v1/systems/current");
  setSystem(system);
};
...
const updateHeader = () => {
  ...
  document.getElementById("user-rank-line").textContent = `${system.className.toUpperCase()} · RANK ${system.rank}`;
...
const renderDashboard = () => {
  ...
  const calHTML =
    days.map((day) => `<div class="cal-day hdr">${day}</div>`).join("") +
...
const renderProfile = () => {
  ...
  document.getElementById("profile-quote").textContent = system.profile.quote;
  document.getElementById("profile-class").textContent = system.className;
  document.getElementById("profile-title").textContent = system.title;
  document.getElementById("profile-guild").textContent = system.profile.guild;
  document.getElementById("profile-presence").textContent = system.profile.presenceLabel;
  document.getElementById("profile-stage").textContent = system.profile.evolutionStage;
  document.getElementById("profile-combat").textContent = formatNumber(system.profile.combatPower);
  document.getElementById("profile-rank-level").textContent = `${system.rank} / ${system.level}`;
  document.getElementById("profile-bio").textContent = system.profile.bio;
  document.getElementById("profile-xp-value").textContent = `${formatNumber(system.xp)} / ${formatNumber(system.xpToNextLevel)}`;
  document.getElementById("profile-xp-bar").style.width = `${xpPct}%`;
  document.getElementById("profile-health-value").textContent = formatNumber(system.vitals.hp);
  document.getElementById("profile-health-bar").style.width = `${Math.max(0, Math.min(100, system.vitals.hp / 10))}%`;
  document.getElementById("profile-mana-value").textContent = formatNumber(system.vitals.mp);
  document.getElementById("profile-mana-bar").style.width = `${Math.max(0, Math.min(100, system.vitals.mp / 10))}%`;
  document.getElementById("profile-energy-value").textContent = formatNumber(system.vitals.energy);
  document.getElementById("profile-energy-bar").style.width = `${Math.max(0, Math.min(100, system.vitals.energy))}%`;
  document.getElementById("profile-str-bar").style.width = `${system.attributes.strength}%`;
  document.getElementById("profile-str-val").textContent = Math.round(system.attributes.strength);
  document.getElementById("profile-int-bar").style.width = `${system.attributes.intelligence}%`;
  document.getElementById("profile-int-val").textContent = Math.round(system.attributes.intelligence);
  document.getElementById("profile-dis-bar").style.width = `${system.attributes.discipline}%`;
  document.getElementById("profile-dis-val").textContent = Math.round(system.attributes.discipline);
...
const questHTML = (quest) => {
  ...
  return `<div class="quest-item ${quest.completed ? "done" : ""}" onclick="completeQuest('${quest.id}')">
    ...
    <div class="quest-rewards">
      ...
      <span class="coin-tag">C ${formatNumber(quest.coinReward)}</span>
    </div>
  </div>`;
};
...
const completeQuest = async (id) => {
  try {
    const system = await request(`/api/v1/quests/${id}/complete`, { method: "POST" });
    setSystem(system);
...
const buyItem = async (id) => {
  try {
    const system = await request(`/api/v1/shop/${id}/purchase`, { method: "POST" });
    setSystem(system);
...
const dismissEvent = async () => {
  try {
    const system = await request("/api/v1/events/dismiss", { method: "POST" });
    setSystem(system);
  } catch (error) {
    showPopup(error.message);
  }
